...
\subsection{Derivatives with Respect to $x$}

\subsubsection{First Derivatives (with respect to $x$)}
...
\subsubsection{Second Derivatives (with respect to $x$)}

\begin{subequations}
\begin{alignat}{3}
\frac{d^2}{dx^2} T_l^m(x) &= a_l^m \left( 2 \frac{d}{dx} T_{l-1}^m(x) + x \frac{d^2}{dx^2} T_{l-1}^m(x) \right) + b_l^m \frac{d^2}{dx^2} T_{l-2}^m(x), & \qquad l \geq m + 2 \\
\frac{d^2}{dx^2} T_{l+1}^l(x) &= c_l \left( 2 \frac{d}{dx} T_l^l(x) + x \frac{d^2}{dx^2} T_l^l(x) \right), & \qquad l \geq 0 \\
\frac{d^2}{dx^2} T_l^l(x) &= d_l \left( -\frac{1}{u^3} T_{l-1}^{l-1}(x) - 2 \frac{x}{u} \frac{d}{dx} T_{l-1}^{l-1}(x) + u \frac{d^2}{dx^2} T_{l-1}^{l-1}(x) \right), & \qquad l \geq 1
\end{alignat}
\end{subequations}

To initialize the recurrence, we need the quantities $T_0^0(x)$, $d T_0^0(x) / dx$, and $d^2 T_0^0(x) / dx^2$. The latter
two terms are zero for all normalizations.

\subsection{Derivatives with Respect to $\theta$}

\subsubsection{First Derivatives (with respect to $\theta$)}
...
\subsubsection{Second Derivatives (with respect to $\theta$)}

\subsubsection{Higher order Derivatives (with respect to $\theta$)}
...
\end{alignat}
A general ALF $T_l^m(\cos{\theta})$ (normalized or unnormalized) will then obey,
\begin{alignat}{3}
\frac{d^k}{d\theta^k} T_0^0(\cos{\theta}) &= 0 \\
\frac{d^k}{d\theta^k} T_l^0(\cos{\theta}) &= -\eta_{CS} e_l \frac{d^{k-1}}{d\theta^{k-1}} T_l^1(\cos{\theta}), & \qquad l \geq 1 \\
\frac{d^k}{d\theta^k} T_l^m(\cos{\theta}) &= \eta_{CS} \left[ f_l^m \frac{d^{k-1}}{d\theta^{k-1}} T_l^{m-1}(\cos{\theta}) - g_l^m \frac{d^{k-1}}{d\theta^{k-1}} T_l^{m+1}(\cos{\theta}) \right], & \qquad l \geq 1, m > 0
\end{alignat}
with (for normalized ALFs)
\begin{subequations}
\begin{alignat}{3}
e_l &= \frac{A_l^0}{A_l^1} \sqrt{l(l+1)}, & \qquad l \geq 1 \\
f_l^m &= \frac{1}{2} \sqrt{(l+m)(l-m+1)} \frac{A_l^m}{A_l^{m-1}}, & \qquad l \geq 1, m > 0 \\
g_l^m &= \frac{1}{2} \sqrt{(l+m+1)(l-m)} \frac{A_l^m}{A_l^{m+1}}, & \qquad l \geq 1, m > 0
\end{alignat}
\end{subequations}
These equations can be applied recursively to find any derivative order
$k$. The factors $e_l,f_l^m,g_l^m$ are computed and stored in the precomputing step.
These factors are given in Table~\ref{tab:recurrence2} for the different normalizations.

\begin{table}[ht]
\centering
\scriptsize
\caption{Coefficients of recursive derivative recurrence relations (with respect to $\theta$).}
\begin{tabular}{@{}ccccc@{}}
\toprule
Normalization & Notation & $e_l$ & $f_l^m$ & $g_l^m$ \\
\midrule
Unnormalized & $P_l^m(x)$ & $1$ & $\frac{1}{2}(l+m)(l-m+1)$ & $\frac{1}{2}$ \\
Schmidt & $S_l^m(x)$ & $\sqrt{\frac{l(l+1)}{2}}$ & $\left\{ \begin{array}{cc} \frac{\sqrt{2}}{2} \sqrt{(l+m)(l-m+1)} & m = 1 \\ \frac{1}{2} \sqrt{(l+m)(l-m+1)} & m > 1 \end{array} \right.$ & $\frac{1}{2} \sqrt{(l+m+1)(l-m)}$ \\
Spherical Harmonic & $Y_l^m(x)$ & $\sqrt{l(l+1)}$ & $\frac{1}{2} \sqrt{(l+m)(l-m+1)}$ & $\frac{1}{2} \sqrt{(l+m+1)(l-m)}$ \\
Full & $N_l^m(x)$ & $\sqrt{l(l+1)}$ & $\frac{1}{2} \sqrt{(l+m)(l-m+1)}$ & $\frac{1}{2} \sqrt{(l+m+1)(l-m)}$ \\
$4\pi$ & $R_l^m(x)$ & $\sqrt{2l(l+1)}$ & $\left\{ \begin{array}{cc} \frac{\sqrt{2}}{2} \sqrt{(l+m)(l-m+1)} & m = 1 \\ \frac{1}{2} \sqrt{(l+m)(l-m+1)} & m > 1 \end{array} \right.$ & $\frac{1}{2} \sqrt{(l+m+1)(l-m)}$ \\
\bottomrule
\end{tabular}
\label{tab:recurrence2}
\end{table}
...
\section{First few Associated Legendre Functions}

\begin{table}[ht]
\centering
\tiny
\caption{First few associated Legendre functions and their derivatives without Condon-Shortley Phase (for testing purposes)}
\begin{tabular}{@{}cccccc@{}}
\toprule
Degree $l$ & Order $m$ & $P_l^m$ & $S_l^m$ & $\partial_{\theta} S_l^m$ & $\partial_{\theta}^2 S_l^m$ \\
\midrule
0 & 0 & 1 & 1 & 0 & 0 \\
1 & 0 & $\cos{\theta}$ & $\cos{\theta}$ & $-\sin{\theta}$ & $-\cos{\theta}$ \\
1 & 1 & $\sin{\theta}$ & $\sin{\theta}$ & $\cos{\theta}$ & $-\sin{\theta}$ \\
2 & 0 & $\frac{1}{2} \left( 3 \cos^2{\theta} - 1 \right)$ & $\frac{1}{2} \left( 3 \cos^2{\theta} - 1 \right)$ & $-3 \cos{\theta} \sin{\theta}$ & $-3 \left( \cos^2{\theta} - \sin^2{\theta} \right)$ \\
2 & 1 & $3 \cos{\theta} \sin{\theta}$ & $\sqrt{3} \cos{\theta} \sin{\theta}$ & $\sqrt{3} \left( \cos^2{\theta} - \sin^2{\theta} \right)$ & $-4 \sqrt{3} \cos{\theta} \sin{\theta}$ \\
2 & 2 & $3 \sin^2{\theta}$ & $\frac{\sqrt{3}}{2} \sin^2{\theta}$ & $\sqrt{3} \cos{\theta} \sin{\theta}$ & $\sqrt{3} \left( \cos^2{\theta} - \sin^2{\theta} \right)$ \\
3 & 0 & $\frac{1}{2} \cos{\theta} \left( 5\cos^2{\theta} - 3 \right)$ & $\frac{1}{2} \cos{\theta} \left( 5\cos^2{\theta} - 3 \right)$ & $\frac{1}{2} \sin{\theta} \left( 3 - 15\cos^2{\theta} \right)$ & $\frac{3}{2} \cos{\theta} \left( 1 - 5\cos^2{\theta} + 10\sin^2{\theta} \right)$ \\
3 & 1 & $-\frac{3}{2} \sin{\theta} \left( 1 - 5\cos^2{\theta} \right)$ & $-\frac{\sqrt{6}}{4} \sin{\theta} \left( 1 - 5\cos^2{\theta} \right)$ & $-\sqrt{\frac{3}{2}} \cos{\theta} \left[ \frac{1}{2} \left(1 - 5\cos^2{\theta}\right) + 5\sin^2{\theta} \right]$ & $-\frac{1}{4} \sqrt{\frac{3}{2}} \sin{\theta} \left[ 23 + 45\left(2\cos^2{\theta} - 1 \right)\right]$ \\
3 & 2 & $15 \cos{\theta} \sin^2{\theta}$ & $\frac{\sqrt{15}}{2} \cos{\theta} \sin^2{\theta}$ & $\frac{\sqrt{15}}{4} \sin{\theta} \left[ 1 + 3 \left( 2\cos^2{\theta} - 1\right)\right]$ & $-\frac{\sqrt{15}}{8} \left[ \cos{\theta} - 9 \left( 4\cos^3{\theta} - 3\cos{\theta}\right)\right]$ \\
3 & 3 & $15 \sin^3{\theta}$ & $\frac{15}{6 \sqrt{10}} \sin^3{\theta}$ & $\frac{3}{2} \sqrt{\frac{5}{2}} \cos{\theta} \sin^2{\theta}$ & $\frac{3}{4} \sqrt{\frac{5}{2}} \sin{\theta} \left[ 1 + 3 \left( 2\cos^2{\theta} - 1\right)\right]$ \\
\bottomrule
\end{tabular}
\label{tab:ALF}
\end{table}
